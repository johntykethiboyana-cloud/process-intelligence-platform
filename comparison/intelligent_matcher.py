...
class IntelligentMatcher:
    ...
    def extract_action(self, text):
        actions = [
            "create","click","select","update","delete",
            "approve","reject","save","submit","search",
            "open","login","upload","download","assign"
        ]
    ...
    def extract_application(self, text):
        apps = [
            "gcss","sap","aris","sharepoint",
            "outlook","excel","salesforce","power bi","teams"
        ]
        t = self.normalize(text)
        for a in apps:
            if a in t:
    ...
    def detailed_score(self, l4_text, sop_text):
        ...
        kw1 = self.extract_keywords(l4_text)
        kw2 = self.extract_keywords(sop_text)
